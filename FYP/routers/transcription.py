...
async def process_video_transcription(task_id: str, video_path: str, youtube_id: Optional[str] = None):
    ...
    try:
        ...
        if youtube_id:
            task_status[task_id]["progress"] = 10
            segments, ts_text, lang, raw_text = await get_youtube_transcript(youtube_id)
            if segments:
                task_status[task_id]["progress"] = 100
                transcript_file = os.path.join(settings.TRANSCRIPTS_DIR, f"{task_id}_transcript.txt")
                ...
                task_status[task_id] = {
                    ...
                    "result": {
                        ...
                        "full_transcript": ts_text,
                        "raw_text": raw_text,
                        "detected_language": lang,
                        "transcript_file_url": f"/downloads/transcripts/{os.path.basename(transcript_file)}"
                    }
                }
                return
        ...
        task_status[task_id]["progress"] = 20
        segments, ts_text, lang, raw_text = await transcribe_video(video_path)
        if segments:
            task_status[task_id]["progress"] = 95
            transcript_file = os.path.join(settings.TRANSCRIPTS_DIR, f"{task_id}_transcript.txt")
            with open(transcript_file, 'w', encoding='utf-8') as f:
...
@router.post("/transcribe/youtube", response_model=ProcessingStatusResponse, summary="Transcribe a YouTube video")
async def transcribe_youtube_video(request: YouTubeRequest, background_tasks: BackgroundTasks):
    task_id = str(uuid.uuid4())
    downloaded_file, _, _, video_id = await download_video(request.youtube_url)
    if not downloaded_file or not video_id:
        raise HTTPException(status_code=400, detail="Failed to download YouTube video")
    background_tasks.add_task(process_video_transcription, task_id, downloaded_file, video_id)
    return ProcessingStatusResponse(task_id=task_id, status="processing", progress=0, result_url=None)


@router.post("/transcribe/upload", response_model=ProcessingStatusResponse, summary="Transcribe an uploaded video")
async def transcribe_uploaded_video(background_tasks: BackgroundTasks, file: UploadFile = File(...)):
    task_id = str(uuid.uuid4())
    file_path = os.path.join(settings.TEMP_DIR, f"{task_id}_{file.filename}")
    with open(file_path, 'wb') as f:
...
@router.get("/transcribe/status/{task_id}", response_model=ProcessingStatusResponse, summary="Check transcription status")
async def check_transcription_status(task_id: str):
    if task_id not in task_status:
        raise HTTPException(status_code=404, detail=f"Task {task_id} not found")
    ...
    if info["status"] == "completed":
        resp.result_url = info["result"]["transcript_file_url"]
    return resp


@router.get("/transcribe/result/{task_id}", response_model=TranscriptionResponse, summary="Get transcription results")
async def get_transcription_result(task_id: str):
    if task_id not in task_status:
        raise HTTPException(status_code=404, detail=f"Task {task_id} not found")
    info = task_status[task_id]
    if info["status"] != "completed":
        raise HTTPException(status_code=400, detail=f"Task is {info['status']}, not completed")
    return TranscriptionResponse(**info["result"])


@router.get("/transcripts/{filename}", summary="Download a transcript file")
async def download_transcript_file(filename: str):
    path = os.path.join(settings.TRANSCRIPTS_DIR, filename)
