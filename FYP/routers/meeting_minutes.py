...
#         # Generate meeting minutes with simplified method - just pass full text
#         meeting_minutes_data = await generate_meeting_minutes(
#             full_text,
#             video_info["title"]
#         )
...
#         # Format preview text - simple version since format_minutes_for_display is removed
#         minutes_preview = ""
#         if "title" in meeting_minutes_data:
#             minutes_preview += f"# {meeting_minutes_data['title']}\n\n"
#         if "date" in meeting_minutes_data:
#             minutes_preview += f"Date: {meeting_minutes_data['date']}\n\n"
#         if "participants" in meeting_minutes_data:
#             minutes_preview += "Participants: " + ", ".join(meeting_minutes_data['participants']) + "\n\n"
#         if "agenda" in meeting_minutes_data:
#             minutes_preview += "Agenda items: " + ", ".join(meeting_minutes_data['agenda'][:3])
#             if len(meeting_minutes_data['agenda']) > 3:
#                 minutes_preview += f" (+ {len(meeting_minutes_data['agenda']) - 3} more)"
#             minutes_preview += "\n\n"
...
async def process_meeting_minutes_generation(meeting_minutes_task_id: str, video_task_id: str, output_format: str):
    ...
    try:
        ...
        # Rename and move file to include task ID
        file_extension = ".md" if output_format.lower() == "md" else ".txt"
        new_filename = f"{meeting_minutes_task_id}_meeting_minutes{file_extension}"
        new_path = os.path.join(meetings_dir, new_filename)
